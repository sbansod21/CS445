...
class BSTreeP5<T>
{
	...
	private int countLeaves( BSTNode<T> root)
	{	
		if (root == null) 
		{
			return 0; 
	...
	@SuppressWarnings("unchecked")
	public BSTreeP5(  BSTreeP5 other )
	{
		BSTreeHelper(other.root);
	}

	private BSTreeP5 BSTreeHelper(BSTNode<T> root)
	{
		if(root == null)
		{
			return null;
			...
			if(root.left != null)
			{
				BSTreeHelper(root.left);
			}
			this.add(root.key);
			if(root.right != null)
			{
				BSTreeHelper(root.right);
			}
		}
